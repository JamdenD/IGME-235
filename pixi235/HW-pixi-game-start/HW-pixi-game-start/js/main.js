...
function createLabelsAndButtons() {
	...
	gameOverScoreLabel.y = sceneHeight - 250;
...
function fireBullet(e){
	...
	let mouseY = e.clientY - rectsIntersect.y;
	...
	if (score >= 5){
		...
		let b1 = new Bullet(0xFFFFFF, ship.x - pewDistance, ship.y);
		let b2 = new Bullet(0xFFFFFF, ship.x, ship.y);
		let b3 = new Bullet(0xFFFFFF, ship.x + pewDistance, ship.y);
...
function end(){
	paused = true;
	// clear out level
	circles.forEach(c=>gameScene.removeChild(c)); //concise arrow funciton with no brackets and no return
	circles = [];

	bullets.forEach(b=>gameScene.removeChild(b)); //same
	bullets = [];
